(define (domain delivery)
  ...
  (:action MOVE-CARRIER
    ...
    :precondition (and (at ?carrier ?loc-from) (in-city ?loc-from ?city) (in-city ?loc-to ?city)
                  )
    :effect       (and (not (at ?carrier ?loc-from)) (at ?carrier ?loc-to)
                  )
  )

  (:action FLY-AGENT
    :parameters   (?agent - agent ?loc-from - base ?loc-to - base)
    :precondition (at ?agent ?loc-from)
    :effect        (and (not (at ?agent ?loc-from)) (at ?agent ?loc-to)
                    )
  )
)
